<template>
  <div class="page">
    ...
    <section class="card">
      ...
      <ol class="steps">
        <li>
          <b>设定空间</b>：进入「工作台」，点击左侧对象树「空间」节点（或点 3D 场景空白处），在右侧面板输入行数与列数（3~300）并生成。
        </li>
        <li>
          <b>布置环境</b>：在右侧「绘制工具」切换到 障碍 / 出口 / 人员，点击 3D 场景中的格子涂绘；
          支持随机生成 N 人。出口至少 1 个。
        </li>
        <li>
          <b>选择算法并仿真</b>：点对象树「仿真」节点，右侧选择算法（多源距离场为默认推荐），
          点「▶ 开始疏散」播放 3D 动画；「结果」节点下可查看统计、路径、热力图与出口分流。
        </li>
        <li>
          <b>保存与复用</b>：顶部「导出场景」可下载 JSON，「导入场景」可载入；「导出报告」一键生成
          全算法对比报告（浏览器打开后 Ctrl+P 可另存为 PDF）。
        </li>
      </ol>
    </section>
    ...
    <section class="card">
      ...
      <p class="hint">想对比全部算法？在工作台点「导出报告」，一键跑 6 种算法并生成对比报告（另存 PDF）；也可直接在「结果」面板查看统计与分流。</p>
    </section>
